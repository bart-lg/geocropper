            ...
            if tile["platform"].startswith("Sentinel"):
                ...
                if sentinel.ready_for_download(tile['productId']):
                    ...
                    if granule.startswith("S1") and asf.download_S1_tile(granule, config.bigTilesDir):
                        ...
                        if tile['lastDownloadRequest'] == None or date_older_than_24h(tile['lastDownloadRequest']):

                            print("Last successful download request older than 24h or non-existing.")
                            print("Repeat download request...")

                            last_request = minutes_since_last_download_request()

                            if last_request == None or last_request > config.copernicusRequestDelay:

                                if sentinel.request_offline_tile(tile['productId']) == True:
                                    ...
                                    # Request error
                                    db.clear_last_download_request_for_tile(tile["rowid"])
